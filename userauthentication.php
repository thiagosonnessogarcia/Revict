<?php
    ...
    $pass = "";
    $banco = "revict";
    $conexao = mysqli_connect($host, $user, $pass) or die("FALHA NA CONEXÃO COM O BANCO");
    mysqli_select_db($conexao, $banco) or die("Não CONECTADO NO BANCO");
?>
...
<head>
	...
	<script type="text/javascript">
		...
		function loginsucessfully(){
			setTimeout("window.location= 'logado/index.php'", 3000);
		}
...
$email = $_POST['email'];
$senha = $_POST['senha'];
$sql = ("SELECT * FROM usuario WHERE email = '$email' and senha = '$senha' ") or die(mysqli_error($banco, "Não foi possivel se conectar ao BANDO DE DADOS!!"));
...
if (!$row || mysqli_num_rows($sql) == 0) {
  ...
  echo "<center><h1>Login Realizado com Sucesso!!!</h1></center>";
  echo "<center><h2>Você será redirecionado para sua Home Page</h2></center>;";
  echo "<script>loginsucessfully()</script>";
} else {
  echo "<center><h1>Nome de usuário ou senha inválidos !!!</h1></center>";
  echo "<center><h2>Aguarde um instante para tentar novamente </h2></center>";
